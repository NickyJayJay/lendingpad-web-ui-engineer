...
| `npm run icons` | Regenerate the icon font from the SVGs in `assets/svg/`. |
...
│   └── fonts/icons/          Generated icon font (WOFF2/WOFF/TTF)
├── vite.config.js
...
- **Icons**: All icons are served via a generated icon font. Add a new icon by dropping an optimized SVG into `assets/svg/` and running `npm run icons`.
